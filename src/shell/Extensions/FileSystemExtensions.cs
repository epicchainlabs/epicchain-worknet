...
namespace EpicChainShell
{
  static class FileSystemExtensions
  {
    ...
    public static string ResolveFileName(this IFileSystem fileSystem, string fileName)
    {
      if (!fileSystem.Path.IsPathFullyQualified(fileName))
      {
        fileName = fileSystem.Path.Combine(fileSystem.Directory.GetCurrentDirectory(), fileName);
    ...
    public static Connections LoadConnections(this IFileSystem fileSystem)
    {
      string rootPath = fileSystem.Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile), ".neo");
      string connectionsFile = fileSystem.Path.Combine(rootPath, "connections.json");
      var connections = new Connections();
      string json = string.Empty;
      if (File.Exists(connectionsFile))
      {
        json = File.ReadAllText(connectionsFile);
        connections = Connections.FromJson(json);
      }
      return connections;
    }

    public static void SaveConnections(this IFileSystem fileSystem, Connections connections)
    {
      string rootPath = fileSystem.Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile), ".neo");
      string connectionsFile = fileSystem.Path.Combine(rootPath, "connections.json");
      string json = connections.ToJson();
