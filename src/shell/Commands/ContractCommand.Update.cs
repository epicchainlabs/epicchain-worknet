...
namespace NeoShell.Commands
{
  ...
  {
    [Command("update", Description = "Update a contract")]
    internal class Update
    {
      ...
      internal async Task<int> OnExecuteAsync(CommandLineApplication app, IConsole console)
      {
        try
        {
          if (Account.Equals(ExpressChainExtensions.GENESIS, StringComparison.OrdinalIgnoreCase))
          {
            console.WriteLine("Cannot use genesis account to deploy/update contract");
            return 0;
          }
